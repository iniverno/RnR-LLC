...
public:
  ...
  physical_address_t bitSelect(int small, int big,int small2, int big2) const; 
  ...
  //JORGE
  void onlyPageAddress() { m_address >>= RubyConfig::pageSizeBits(); }
...
inline
physical_address_t Address::bitSelect(int small, int big, int small2, int big2) const // rips bits inclusive
{
  physical_address_t mask;
  assert(big >= small);
  assert(small2 >= big);
  ...
  physical_address_t partial2,partial1;
  
  if (big >= ADDRESS_WIDTH - 1) {
    return (m_address >> small);
  } else {
    if(big2 >= ADDRESS_WIDTH - 1)
    {
      partial2= (m_address >> small2);
      partial2 <<= big+1;
      
    } else
    {
      mask = ~((physical_address_t)~0 << (big2 + 1));
    // FIXME - this is slow to manipulate a 64-bit number using 32-bits
      partial2 = (m_address & mask);
      partial2= (partial2 >> small2);
      partial2 <<= big+1;
    }
    
    mask = ~((physical_address_t)~0 << (big + 1));
    // FIXME - this is slow to manipulate a 64-bit number using 32-bits
    partial1 = (m_address & mask); 
   
   
   // cout << "partial1: " << hex << partial1 << endl;
   // cout << "partial2: " << hex << partial2 << endl;
    return (partial2|partial1) >> small;
  }
}
...
integer_t Address::memoryModuleIndex() const
{
  ...
  if(g_GROUP_L2BANKS_TO_MEM) index = bitSelect(RubyConfig::dataBlockBits(), g_PAGE_SIZE_BITS + g_NUM_L2_BANKS_PER_CHIP_BITS - RubyConfig::memoryBits()-1, g_PAGE_SIZE_BITS + g_NUM_L2_BANKS_PER_CHIP_BITS, ADDRESS_WIDTH-1); 
  else index = bitSelect(RubyConfig::dataBlockBits()+RubyConfig::memoryBits(), ADDRESS_WIDTH);
