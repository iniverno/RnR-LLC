        ...
        if ((param_type == CoherenceRequestType_UPGRADE)) {
          ...
          if ((param_type == CoherenceRequestType_PUTX)) {
            ...
            if ((param_type == CoherenceRequestType_PUTS)) {
              ...
              DEBUG_SLICC(MedPrio, "../protocols/MSI_MOSI_inclusive-L2cache.sm:448: ", param_addr);
;
              DEBUG_SLICC(MedPrio, "../protocols/MSI_MOSI_inclusive-L2cache.sm:449: ", param_type);
;
                            cerr << "Runtime Error at ../protocols/MSI_MOSI_inclusive-L2cache.sm:450, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("Invalid L1 forwarded request type") << ", PID: " << getpid() << endl;
