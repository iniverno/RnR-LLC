...
void Throttle::addLinks(const Vector<MessageBuffer*>& in_vec, const Vector<MessageBuffer*>& out_vec)
{
  ...
  for (int i=0; i<in_vec.size(); i++) {
    ...
    //cout << "En el throttle " << m_node <<", " << throttle_id << " a–adida la vnet " << i << endl;
  }
...
{
	//cout << "me despierto " << int_to_string(m_node) << "   " << int_to_string(g_eventQueue_ptr->getTime()) <<  endl;
 ...
 //if(bw_remaining <= 0) cout << "no tenemos ancho de banda en el throttle: " << m_node << " " << vnet << "   quedan  " <<m_units_remaining[vnet]<< endl;
    while ((bw_remaining > 0) && ((m_in[vnet]->isReady()) || (m_units_remaining[vnet] > 0)) && m_out[vnet]->areNSlotsAvailable(1)) {
      ...
      if (m_units_remaining[vnet] == 0 && m_in[vnet]->isReady()) {
       ...
       /* cout <<  "throttle: " + int_to_string(m_node) + ", " + int_to_string(throttle_id) +
                  ...
                  + " time: " + int_to_string(g_eventQueue_ptr->getTime()) + "." << endl;
    */
        // Move the message
        //cout << " LL:" <<  m_link_latency << endl;
      ...
      //if(m_node==0)
      //cout << "en el throttle " << int_to_string(m_node) <<", " << throttle_id << " " << vnet << "  quedan: " << int_to_string( m_units_remaining[vnet]) << endl;
    }
  ...
  if ((bw_remaining > 0) && !schedule_wakeup) {
    ...
    //cout << "not scheduled again " << int_to_string(m_node) << endl;
  } else {
    DEBUG_MSG(NETWORK_COMP,LowPrio,*this);
    DEBUG_MSG(NETWORK_COMP,LowPrio,"scheduled again");
    //cout <<  int_to_string(m_node) <<  "   scheduled again" << endl;
