...
//JORGE
 string filename;
 if(!g_IN_HERMES)
   filename = "/home/jalbe/comun/gems/ruby/network/simple/Network_Files/";
 else 
  filename = "/home/gaz/jalbe/gems/rubys/";
...
//JORGE
 /* if (g_SIMICS) {
  ...
  cout << "fichero: " << filename.c_str() << endl;
    ...
    while (varStr != "") {
      ...
      if (label == "ext_node" || label == "int_node") {
        ...
        if (label == "ext_node") { // input link to node
          ...
          cout << " ASSERT(nodes[i] >= 0 && nodes[i] < m_nodes);    nodes[i] " << nodes[i] << "m_nodes " << m_nodes << endl;
     ...
     // cout << g_endpoint_bandwidth << " " << string_split(varStr, ':').c_str() <<  endl;
