  ...
  : m_conflicting_histogram(-1)
{
  ...
  m_L15_cache_profiler_ptr = new CacheProfiler("L15_cache");
...
Profiler::~Profiler()
{
  ...
  delete m_L15_cache_profiler_ptr;
...
void Profiler::printStats(ostream& out, bool short_stats)
{
  ...
  out << endl;
m_L1D_cache_profiler_ptr->calculaRatios();
m_L1I_cache_profiler_ptr->calculaRatios();
m_L15_cache_profiler_ptr->calculaRatios();
m_L2_cache_profiler_ptr->calculaRatios();

  m_L1D_cache_profiler_ptr->printStats(out);
  m_L1I_cache_profiler_ptr->printStats(out);
  //m_L2_cache_profiler_ptr->printStats(out);
  m_L15_cache_profiler_ptr->printStats15(out);
  m_L2_cache_profiler_ptr->printStats2(out);
...
void Profiler::clearStats()
{
  ...
  m_L15_cache_profiler_ptr->clearStats();
...
void Profiler::addL1Acceso(const CacheMsg& msg, NodeID id)
{
...
void Profiler::addL1PrefInv( int id, bool inst)
{
  if(inst)
    m_L1I_cache_profiler_ptr->addPrefInv(id);
  else m_L1D_cache_profiler_ptr->addPrefInv(id);
}

void Profiler::addL2Acceso(AccessModeType access_mode, NodeID id)
{
  m_L2_cache_profiler_ptr->addL2Acceso(access_mode, id);
}
...
void Profiler::add15StatSample(const CacheMsg& msg, NodeID id)
{
      m_L15_cache_profiler_ptr->addStatSample(CacheRequestType_to_GenericRequestType(msg.getType()),
                                          msg.getAccessMode(), msg.getSize(), msg.getPrefetch(), id);
   
}

void Profiler::addL15Acceso(const CacheMsg& msg, NodeID id)
{
  m_L15_cache_profiler_ptr->addAcceso(CacheRequestType_to_GenericRequestType(msg.getType()),
                                          msg.getAccessMode(), msg.getSize(), msg.getPrefetch(), id, true);
}
...
void Profiler::addL2StatSample(GenericRequestType requestType, AccessModeType type, int msgSize, PrefetchBit pfBit, NodeID id)
{
  ...
  m_L2_cache_profiler_ptr->addStatSample(requestType, type, msgSize, pfBit, id);
...
int64 Profiler::getL2Accesos(){  return m_L2_cache_profiler_ptr->getAccesos(); }

void Profiler::addL1IAcceso(const CacheMsg& msg, NodeID id)
{
  m_L1I_cache_profiler_ptr->addAcceso(CacheRequestType_to_GenericRequestType(msg.getType()),
                                          msg.getAccessMode(), msg.getSize(), msg.getPrefetch(), id);
}

void Profiler::addL1DAcceso(const CacheMsg& msg, NodeID id)
{
  m_L1D_cache_profiler_ptr->addAcceso(CacheRequestType_to_GenericRequestType(msg.getType()),
                                          msg.getAccessMode(), msg.getSize(), msg.getPrefetch(), id);
}
