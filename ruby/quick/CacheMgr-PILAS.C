...
CacheMgr::CacheMgr(SimicsDriver* a) {
	...
	for(uint i=0; i<NUM_CORES; i++) {
		...
		caches[1][i]= new CacheMemoryQuick( numSetsBitsL2, 8, MachineTypeQuick_L1Cache, this, name, i);
	}
...
//P 				if(!exclusion_L3_L2) {
//P 					assert(caches[2][bank]->lookup(*addr).m_Sharers.bitMap[proc + nCores * (!isFetch)]);
//P 					assert(caches[2][bank]->testCacheAccess(*addr, CacheRequestTypeQuick_LD));
//P 					caches[2][bank]->lookup(*addr).m_Sharers.del(proc + nCores * (!isFetch));
//P 					caches[2][bank]->lookup(*addr).m_instr = isFetch;
//P 				}
			   }
...
//P 					caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_Sharers.del(proc + nCores * isFetch);  //dir preciso
				} 
...
//P 							else {
//P 								assert(caches[2][auxBank]->lookup(aux2).m_Sharers.bitMap[proc + nCores * 2]);
//P 								//assert(caches[2][bank]->testCacheAccess(aux2, CacheRequestTypeQuick_LD));
//P 								
//P 								caches[2][auxBank]->lookup(aux2).m_Sharers.del(proc + nCores*2);   //directorio preciso	
//P 								caches[2][auxBank]->m_replacementPolicy_ptr->reusedL1 = caches[1][proc]->lookup(aux2).m_reused;
//P 								caches[2][auxBank]->replacementLower(aux2);
//P 							}
...
//P 						if(!exclusion_L3_L2)  {
//P 							//assert(caches[2][bank]->testCacheAccess(aux, CacheRequestTypeQuick_LD));
//P 							caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_Sharers.add(proc + nCores*2);   //directorio preciso
//P 						}
...
//P 			if(exclusion_L3_L2)
//P 				if(caches[2][bank]->testCacheAccess(*addr, request))  
//P 						caches[2][bank]->deallocate(*addr);
	...
	/* PILAS_L2   */	
		if(g_PILAS && !isPrefetch && !isFetch) {
			uint aux2=caches[1][0]->addressToCacheSet(*addr);
		
			if((aux2 && 0xf) == 1) {
				
				if(!PILAS_L2[0][thread]->tryCacheAccess(*addr, request, thread)) {
					if(!PILAS_L2[0][thread]->cacheAvail(*addr)) {
						Address aux = PILAS_L2[0][thread]->cacheProbe(*addr);
						PILAS_L2[0][thread]->deallocate(aux);
					}
					PILAS_L2[0][thread]->allocate(*addr, thread);
				}
				
				if(PILAS_L2[1][thread]->tryCacheAccess(*addr, request, thread)) {
						PILAS_L2[1][thread]->deallocate(*addr);
						assert(!PILAS_L2[2][thread]->tryCacheAccess(*addr, request, thread));
						if(!PILAS_L2[2][thread]->cacheAvail(*addr)) {
							Address aux = PILAS_L2[2][thread]->cacheProbe(*addr);
							PILAS_L2[2][thread]->deallocate(aux);
						}
						PILAS_L2[2][thread]->allocate(*addr, thread);
					}
					else {
						if(!PILAS_L2[2][thread]->tryCacheAccess(*addr, request, thread)) {
							if(!PILAS_L2[1][thread]->cacheAvail(*addr)) {
								Address aux = PILAS_L2[1][thread]->cacheProbe(*addr);
								PILAS_L2[1][thread]->deallocate(aux);
							}
							PILAS_L2[1][thread]->allocate(*addr, thread);
						}
					}
...
//P 			if(!exclusion_L3_L2) {
//P 				//assert(caches[2][bank]->testCacheAccess(*addr, CacheRequestTypeQuick_LD));
//P 				caches[2][bank]->lookup(*addr).m_Sharers.add(proc + nCores*isFetch);
//P 				caches[2][bank]->lookup(*addr).m_instr = isFetch;
//P 			} //else assert(!caches[2][bank]->testCacheAccess(*addr, CacheRequestTypeQuick_LD));
...
//P 					if(!exclusion_L3_L2) {
//P 						assert(caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_Sharers.bitMap[proc + nCores * wasFetch]);
//P 						caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_Sharers.del(proc + nCores*wasFetch);   //directorio preciso
//P 					}
...
//P 				else {
//P 					//assert(caches[2][bank]->testCacheAccess(aux, CacheRequestTypeQuick_LD));
//P 					assert(caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_Sharers.bitMap[proc + nCores * 2]);
//P 					caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_Sharers.del(proc + 2*nCores);   //directorio preciso					
//P 					caches[2][map_Address_to_L2(aux).num]->replacementLower(aux);
//P 				}
