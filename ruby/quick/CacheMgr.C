...
uint CacheMgr::access(memory_transaction_t* mt, bool isPrefetch, Address prefetchAddress, uint prefetchThread) {
		...
		if(!isPrefetch) {
			  ...
			  if(caches[0][proc + nCores*(!isFetch)]->testCacheAccess(*addr, request)) {
				...
				if(inclusion_L3_lower && !g_PILAS) {// Con inclusion
					...
					assert(!caches[0][proc + nCores * (!isFetch)]->testCacheAccess(*addr, CacheRequestTypeQuick_LD));//P 
			...
			if(!caches[0][proc + nCores*isFetch]->cacheAvail(*addr)) {
				...
				if(inclusion_L3_lower && !g_PILAS) {
					...
					assert(!caches[0][proc + nCores * isFetch]->testCacheAccess(*addr, CacheRequestTypeQuick_LD));//P 
					caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_onFly = true;
				}
				...
				if(inclusion_L2_L1) {
					...
					if(!caches[1][proc]->testCacheAccess(aux, request)) {
						...
						if(!caches[1][proc]->cacheAvail(aux)) {
							...
							if(exclusion_L3_L2)
							{
								...
								for(uint i=0; i<nCores && !flag; i++) {
									...
									//if(caches[1][i]->testCacheAccess(aux2,CacheRequestTypeQuick_LD)) flag=true; //lo estamos expulsando de la L2
								}
								...
								if(!flag) {//P 
									...
									if(!caches[2][auxBank]->testCacheAccess(aux2,request)) {
										...
										if(caches[2][auxBank]->m_overAssocAddress != Address(0)) {
											invalidateCopies(caches[2][auxBank]->m_overAssocAddress);
											caches[2][auxBank]->m_overAssocAddress = Address(0);
										}
								...
								if(inclusion_L3_lower && !g_PILAS)
								{
									...
									//!!! Esta accion la retrasamos hasta el final porque si no parece que la entrada no la tiene nadie en la jerarquia!!!
									caches[2][auxBank]->lookup(aux2).m_Sharers.del(proc + nCores*2);   //directorio preciso	//P 
									//assert(caches[1][proc]->testCacheAccess(*addr, CacheRequestTypeQuick_LD));//P 
									caches[2][auxBank]->lookup(aux2).m_onFly = true;
						...
						if(inclusion_L3_lower && !g_PILAS)  {//P 
							...
							caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_onFly = false;
						}//P 
						if(!inclusion_L3_lower && !g_PILAS)  {//P 
							//assert(caches[2][bank]->testCacheAccess(aux, CacheRequestTypeQuick_LD));//P 
							if(caches[2][map_Address_to_L2(aux).num]->testCacheAccess(aux, CacheRequestTypeQuick_LD))
					...
					assert(caches[1][proc]->testCacheAccess(aux, CacheRequestTypeQuick_LD));
					if(inclusion_L3_lower) {
						assert(caches[2][map_Address_to_L2(aux).num]->testCacheAccess(aux, CacheRequestTypeQuick_LD));
						assert(!caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_Sharers.bitMap[proc] && !caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_Sharers.bitMap[proc + nCores]);
						assert(caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_Sharers.bitMap[proc + nCores*2]);
					}
				}
			} 
	...
	/************* L2 miss *************************/
				...
				if(exclusion_L3_L2) {
					...
					if(!flag) {
						...
						if(!caches[2][map_Address_to_L2(aux).num]->testCacheAccess(aux,request)) {
							...
							if(caches[2][map_Address_to_L2(aux).num]->m_overAssocAddress != Address(0)) {
								invalidateCopies(caches[2][map_Address_to_L2(aux).num]->m_overAssocAddress);
								caches[2][map_Address_to_L2(aux).num]->m_overAssocAddress = Address(0);
							} 
						}
					...
					//if(!flag && !caches[2][map_Address_to_L2(aux).num]->lookup(aux).m_onFly) {
					if(!flag) {
			...
			assert(caches[0][proc + isFetch*nCores]->testCacheAccess(*addr, CacheRequestTypeQuick_LD));
		...
		/************* L3  *************************/
				...
				falloCorr = true;
				if(caches[2][bank]->m_overAssocAddress != Address(0)) {
					assert(caches[2][bank]->m_overAssocAddress != 	*addr);
					invalidateCopies(caches[2][bank]->m_overAssocAddress);
					caches[2][bank]->m_overAssocAddress = Address(0);
				}
...
bool CacheMgr::checkAddress(Address addr) {

	uint nCores = RubyConfig::numberOfProcsPerChip() / g_NUM_SMT_THREADS;

	for(uint i=0; i<nCores; i++) {
		if(caches[0][i]->testCacheAccess(addr, CacheRequestTypeQuick_LD)) {
			assert(caches[2][map_Address_to_L2(addr).num]->lookup(addr).m_Sharers.bitMap[i]); 
		}
		else  {
			assert(!caches[2][map_Address_to_L2(addr).num]->lookup(addr).m_Sharers.bitMap[i]);
		}
		
		if(caches[0][i+nCores]->testCacheAccess(addr, CacheRequestTypeQuick_LD)) {
			assert(caches[2][map_Address_to_L2(addr).num]->lookup(addr).m_Sharers.bitMap[i + nCores ]);
		}
		else {
			assert(!caches[2][map_Address_to_L2(addr).num]->lookup(addr).m_Sharers.bitMap[i + nCores]);
		}
		
		if(caches[1][i]->testCacheAccess(addr, CacheRequestTypeQuick_LD)) {
			assert(caches[2][map_Address_to_L2(addr).num]->lookup(addr).m_Sharers.bitMap[i + nCores * 2 ]);
		}
		else {
			assert(!caches[2][map_Address_to_L2(addr).num]->lookup(addr).m_Sharers.bitMap[i + nCores * 2]);
		}
...
void CacheMgr::invalidateCopies(const Address& a) {
	uint bank = map_Address_to_L2(a).num;
	
	for(uint i=0; i<NUM_CORES; i++) {
		if(caches[1][i]->testCacheAccess(a, CacheRequestTypeQuick_LD)) {
			//assert(entryAux.m_Sharers.bitMap[i]);
			caches[1][i]->deallocate(a);
		}
		if(caches[0][i + NUM_CORES]->testCacheAccess(a, CacheRequestTypeQuick_LD)) {
			//assert(entryAux.m_Sharers.bitMap[i]);
			caches[0][i + NUM_CORES]->deallocate(a);
		}
		if(caches[0][i]->testCacheAccess(a, CacheRequestTypeQuick_LD)) {
			//assert(entryAux.m_Sharers.bitMap[i]);
			caches[0][i]->deallocate(a);
		}
	}
	caches[2][bank]->lookup(a).m_Sharers.reset();
}
...
void CacheMgr::clearStats() {
	...
	for(uint i=0; i<NUM_CORES; i++) {
		caches[0][i + NUM_CORES]->clearStats();
	}
		for(uint i=0; i<NUM_CORES; i++) {
		caches[0][i]->clearStats();
	}

	for(uint i=0; i<NUM_CORES; i++) {
		caches[1][i]->clearStats();
	}
...
void CacheMgr::pinta(Address* addr) {
	...
	if(index != -1) {
		...
		int privated=0, reused=0, other=0;
		...
		fich[index] << privated << "\t" << reused << "\t" << other << endl;
