...
#ifndef quickLRRdPolicy_H
#define quickLRRdPolicy_H
...
class quickLRRdPolicy : public AbstractReplacementPolicyQuick {
 public:

  quickLRRdPolicy(Index num_sets, Index assoc, Vector<Vector<CacheEntryQuick> > *a);
  ~quickLRRdPolicy();
...
inline
quickLRRdPolicy::quickLRRdPolicy(Index num_sets, Index assoc, Vector<Vector<CacheEntryQuick> > *a)
...
inline
quickLRRdPolicy::~quickLRRdPolicy()
{
}

inline 
void quickLRRdPolicy::touch(Index set, Index index, Time time, uint proc){
  ...
  if(ant == 0) {
  	...
  	if(ant<17) {
  		...
  		for (unsigned int i=0; i < m_assoc; i++) {
  			if(m_LRUorder[set][i] >= 17 && m_LRUorder[set][i] < ant && m_LRUorder[set][i] != 0) m_LRUorder[set][i]++;
  		}
  		Index victim = victimPriv(set);
  		int victimPosLRU = m_LRUorder[set][victim];
  		for (unsigned int i=0; i < m_assoc; i++) {
...
inline
Index quickLRRdPolicy::getVictim(Index set, uint proc) const {
...
inline
Index quickLRRdPolicy::victimPriv(Index set)  {
...
inline void quickLRRdPolicy::replacementLower(Index set, Index way)  {
	//if((*m_cache)[set][way].m_reused) m_last_ref_ptr[set][way] = g_eventQueue_ptr->getTime();
}

inline 
uint quickLRRdPolicy::whatSubSet(CacheEntryQuick &a) const
{
...
inline int quickLRRdPolicy::posInSubset(int set, int index, CacheEntryQuick &p) {
...
inline uint quickLRRdPolicy::whatSubset(CacheEntryQuick& a) {
	if(!a.m_NRU) return 0;
	else return 1;
}
...
inline
void quickLRRdPolicy::printStats(ostream& out, char* name) 
{
  //out << "select: " << select[0] << "\t"  << select[1] << "\t"  << select[2] << endl; 
cerr << "quickLRRdPolicy::printStats" << endl;
	for(int i=0; i<RubyConfig::numberOfProcsPerChip(); i++) {
		for (int j = 0; j < 2; j++)
...
inline
void quickLRRdPolicy::clearStats() 
{
cerr << "quickLRRdPolicy::clearStats()" << endl;
...
inline
void quickLRRdPolicy::printStats(ostream& out) 
{
  //out << "select: " << select[0] << "\t"  << select[1] << "\t"  << select[2] << endl; 
cerr << "quickLRRdPolicy::printStats" << endl;
}
