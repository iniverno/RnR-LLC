...
#ifndef quickLRUPolicy_3SETc_H
#define quickLRUPolicy_3SETc_H
...
class quickLRUPolicy_3SETc : public AbstractReplacementPolicyQuick {
 public:

  quickLRUPolicy_3SETc(Index num_sets, Index assoc, Vector<Vector<CacheEntryQuick> > *a);
  ~quickLRUPolicy_3SETc();
...
inline
quickLRUPolicy_3SETc::quickLRUPolicy_3SETc(Index num_sets, Index assoc, Vector<Vector<CacheEntryQuick> > *a)
...
inline
quickLRUPolicy_3SETc::~quickLRUPolicy_3SETc()
{
}

inline 
void quickLRUPolicy_3SETc::touch(Index set, Index index, Time time, uint proc){
  ...
  for(uint i=0; i<m_assoc; i++) 
	{	
		CacheEntryQuick &a=(*m_cache)[set][i];
		if(a.m_Sharers.count()==0 && !a.m_reused)aux++;
	}
...
inline
Index quickLRUPolicy_3SETc::getVictim(Index set, uint proc) const {
  //  assert(m_assoc != 0);
...
inline void quickLRUPolicy_3SETc::replacementLower(Index set, Index way)  {
}

inline
void quickLRUPolicy_3SETc::printStats(ostream& out, char* name) 
{
...
inline
void quickLRUPolicy_3SETc::printStats(ostream& out) 
{
	out << "m_nReused: " << (int)m_nReused[17] <<endl;
}

inline 
uint quickLRUPolicy_3SETc::whatSubSet(CacheEntryQuick &a) const
{
