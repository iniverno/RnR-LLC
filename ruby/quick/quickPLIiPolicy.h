...
#ifndef quickPLIiPolicy_H
#define quickPLIiPolicy_H
...
class quickPLIiPolicy : public AbstractReplacementPolicyQuick {
 public:

  quickPLIiPolicy(Index num_sets, Index assoc, Vector<Vector<CacheEntryQuick> > *a);
  ~quickPLIiPolicy();
  ...
  mutable Histogram  m_histoReuse;
  mutable Histogram  m_histoReuseCore[16];
...
inline
quickPLIiPolicy::quickPLIiPolicy(Index num_sets, Index assoc, Vector<Vector<CacheEntryQuick> > *a)
  : AbstractReplacementPolicyQuick(num_sets, assoc)
{
  cerr << "Constructor quickPLIi" << endl;
  srand(time(NULL));
  m_cache = a;
...
inline
quickPLIiPolicy::~quickPLIiPolicy()
{
...
inline 
void quickPLIiPolicy::touch(Index set, Index index, Time time, uint proc){
...
inline
Index quickPLIiPolicy::getVictim(Index set, uint proc) const {
  //  assert(m_assoc != 0);
  uint i=0;
  ...
  for (i=0; i < m_assoc ; i++) {
	...
	steps[k][nEntries[k]++] = m_assoc - m_last_ref_ptr[set][i] + 1;
  }
...
inline 
bool quickPLIiPolicy::subSet(CacheEntryQuick *a, uint subset) const
{
...
inline 
uint quickPLIiPolicy::whatSubSet(CacheEntryQuick *a) const
{
...
inline
void quickPLIiPolicy::printStats(ostream& out, char* name) 
{
	cerr << "Histograma sobre posicion en los aciertos:" << endl;
	cerr << m_histoSpace[0] << endl;
	
	cerr << "Histograma sobre posicion en los aciertos por core:" << endl;
	for(int i=0; i<RubyConfig::numberOfProcessors(); i++) cerr << "Core " << i << ": " << m_histoSpaceCore[i] << endl;
}

inline
void quickPLIiPolicy::printStats(ostream& out) 
{
	cerr << "Histograma sobre posicion en los aciertos:" << endl;
	cerr << m_histoSpace[0] << endl;
