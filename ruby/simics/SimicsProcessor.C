...
//JORGE
  ...
  long SimicsProcessor::getLoads(){return m_sequencer->getLoads(); }
   long SimicsProcessor::getStores(){return m_sequencer->getStores(); }
long SimicsProcessor::getAt(){return m_sequencer->getAt(); }
 long SimicsProcessor::getFetches(){return m_sequencer->getFetches(); }
...
MemoryTransactionResult SimicsProcessor::makeRequest(
  ...
  //JORGE
 /* if(CacheRequestType_ST==getRequestType(mem_trans))
  {m_numStores++;}
  else if(CacheRequestType_LD==getRequestType(mem_trans))
  {m_numLoads++;}*/
...
MemoryTransactionResult SimicsProcessor::makeRequestQuick(
  memory_transaction_t* mem_trans) {
 ...
 if(mem_trans->s.type == Sim_Trans_Instr_Fetch){
    if (PRINT_INSTRUCTION_TRACE && (g_eventQueue_ptr->getTime() >= g_DEBUG_CYCLE )){
      ...
      // write instruction address
      cout << g_eventQueue_ptr->getTime() << " #" << m_proc << "\t" << getInstructionCount() << "\tVPC: " << Address(mem_trans->s.logical_address)
           << " PC: " << Address(mem_trans->s.physical_address)   << endl;
    }
