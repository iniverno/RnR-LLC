...
#ifdef MACHINETYPE_L0Cache
#define MACHINETYPE_L0CACHE_ENUM MachineType_L0Cache
#else
#define MACHINETYPE_L0CACHE_ENUM MachineType_NUM
#endif
...
#ifdef MACHINETYPE_VCache
#define MACHINETYPE_VCACHE_ENUM MachineType_VCache
#else
#define MACHINETYPE_VCACHE_ENUM MachineType_NUM
#endif
...
int map_address_to_L2CacheSet(const Address& addr, int cache_num_set_bits)  
{
  ...
  //JORGE
     return addr.bitSelect(g_DATA_BLOCK_BITS,  g_PAGE_SIZE_BITS - 1 ,
         g_PAGE_SIZE_BITS + g_NUM_L2_BANKS_PER_CHIP_BITS, g_DATA_BLOCK_BITS + g_NUM_L2_BANKS_PER_CHIP_BITS + cache_num_set_bits - 1);
    
 } else
  {  //el entrelazado es por bloque, bloque i+1 -> banco L2 i+1
  	if (MAP_L2BANKS_TO_LOWEST_BITS) {
  //JORGE
      return addr.bitSelect(g_DATA_BLOCK_BITS + g_NUM_L2_BANKS_PER_CHIP_BITS, g_DATA_BLOCK_BITS + g_NUM_L2_BANKS_PER_CHIP_BITS+cache_num_set_bits-1);
...
MachineID map_L1CacheMachId_to_L2Cache(const Address& addr, MachineID L1CacheMachId) 
{
  ...
  {
    if (g_PAGE_INTERLEAVING) 
    {  //el entrelazado es por p‡gina, p‡gina i+1 -> banco L2 i+1
    //JORGE
      L2bank = addr.bitSelect(g_PAGE_SIZE_BITS, g_PAGE_SIZE_BITS + g_NUM_L2_BANKS_PER_CHIP_BITS -1); 
    } else
    {    
      if (MAP_L2BANKS_TO_LOWEST_BITS) 
      {
    //JORGE
      L2bank = addr.bitSelect(RubyConfig::dataBlockBits(),  RubyConfig::dataBlockBits()+RubyConfig::L2CachePerChipBits()-1);
...
inline
MachineID map_L1CacheMachId_to_VCache(const Address& addr, MachineID L1CacheMachId) 
{

  
  MachineID mach = {MACHINETYPE_VCACHE_ENUM, 0};  

  mach.num = L1CacheMachId.num;
  assert(mach.num < RubyConfig::numberOfVCache());
  return mach;
}
...
inline
MachineID map_Address_to_L2(const Address& addr) 
{
  ...
  mach.num = L2bank; // bank #
    
 
  
  assert(mach.num < RubyConfig::numberOfL2Cache() && mach.num>=0);
  return mach;
...
inline
NodeID map_Address_to_L2Node(const Address& addr) 
{
   int L2bank = 0;
  ...
  assert(L2bank < RubyConfig::numberOfL2Cache() && L2bank>=0);
  return L2bank;
}
...
NodeID map_Address_to_DirectoryNode(const Address& addr) 
{
  ...
  if (RubyConfig::memoryBits() > 0) {
    if(g_GROUP_L2BANKS_TO_MEM)
    {  
      dirNode = addr.bitSelect(g_PAGE_SIZE_BITS + g_NUM_L2_BANKS_PER_CHIP_BITS - RubyConfig::memoryBits(), 
         g_PAGE_SIZE_BITS + g_NUM_L2_BANKS_PER_CHIP_BITS-1); 
    } else if(g_PAGE_INTERLEAVING)
    {
      dirNode = addr.bitSelect(g_PAGE_SIZE_BITS, g_PAGE_SIZE_BITS + RubyConfig::memoryBits() -1); 
    }
    else
    {
