...
{
	g_system_ptr->getProfiler()->addL2Acceso(type, proc);
}
...
/* g_system_ptr->getProfiler()->m_L2_cache_profiler_ptr->addAcceso(CacheRequestType_to_GenericRequestType(msg.getType()),
...
void profile_L15Cache_miss( CacheMsg msg, NodeID id)
{
  
  g_system_ptr->getProfiler()->add15StatSample(msg, id);
}

void profileMsgDelay(int virtualNetwork, int delayCycles)
{
	//cout << "retraso: " << delayCycles << endl;
