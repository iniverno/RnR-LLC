  ...
  for(int p=0; p < smt_threads; ++p){
    ...
    for (int i=0; i<keys.size(); i++) {
      ...
      if (current_time - request.getTime() >= g_DEADLOCK_THRESHOLD) {
        ...
        Address aux=request.getAddress();
        aux.makePageAddress();
        //m_chip_ptr->pintame(aux, m_version);
        ERROR_MSG("Aborting");
        deadlock = true;
      }
    }
...
void Sequencer::hitCallback(const CacheMsg& request, DataBlock& data, GenericMachineType respondingMach, int thread) {
  ...
  //commented by JORGE . la llamada a trycacheaccess en doRequest hace que no haya que volver a tocar
...
bool Sequencer::doRequest(const CacheMsg& request) {
  ...
  uint thread = request.getThreadID();
  
  g_system_ptr->getProfiler()->addL1Acceso(request,  (m_chip_ptr->getID()*RubyConfig::numberOfProcsPerChip()+m_version));
  ...
  if(!hit) g_system_ptr->getProfiler()->addL15Acceso(request,  m_chip_ptr->getID()*RubyConfig::numberOfProcsPerChip()+m_version);
  
  //if(!m_chip_ptr->m_L1Cache_L15cacheMemory_vec[m_version]->isTagPresent(line_address(request.getAddress())))
  //	g_system_ptr->getProfiler()->addL2Acceso(request.getAccessMode(),  (m_chip_ptr->getID()*RubyConfig::numberOfProcsPerChip()+m_version)); 
...
void Sequencer::issueRequest(const CacheMsg& request) {
  ...
  if (!found) {
    ...
    if (!REMOVE_SINGLE_CYCLE_DCACHE_FAST_PATH) {
      g_system_ptr->getProfiler()->addPrimaryStatSample(msg, m_chip_ptr->getID()*RubyConfig::numberOfProcsPerChip()+m_version);
...
bool Sequencer::tryCacheAccess(const Address& addr, CacheRequestType type,
  ...
  if (type == CacheRequestType_IFETCH) {
    if (Protocol::m_TwoLevelCache) {
      ret= m_chip_ptr->m_L1Cache_L1IcacheMemory_vec[m_version]->tryCacheAccess(line_address(addr), type, data_ptr, proc);
    } 
    else {
    ...
    if (Protocol::m_TwoLevelCache) {
      ret= m_chip_ptr->m_L1Cache_L1DcacheMemory_vec[m_version]->tryCacheAccess(line_address(addr), type, data_ptr, proc);
    } 
    else {
      ret= m_chip_ptr->m_L1Cache_cacheMemory_vec[m_version]->tryCacheAccess(line_address(addr), type, data_ptr, proc);
    }
  ...
  //JORGE
  //si fallo en L1 contamos como acceso a la L2
  /*
  if(!ret) { 
    g_system_ptr->getPrefetcher(map_Address_to_L2Node(addr))->access(addr, getL1MachineID(m_version));
...
bool Sequencer::getRubyMemoryValue(const Address& addr, char* value,
  ...
  if(g_SIMICS){
    ...
    if (n->m_L1Cache_L1IcacheMemory_vec[m_version]->tryCacheAccess(lineAddr, CacheRequestType_IFETCH, dataPtr, 0)){
      n->m_L1Cache_L1IcacheMemory_vec[m_version]->getMemoryValue(addr, value, size_in_bytes);
      found = true;
    } else if (n->m_L1Cache_L1DcacheMemory_vec[m_version]->tryCacheAccess(lineAddr, CacheRequestType_LD, dataPtr, 0)){
      n->m_L1Cache_L1DcacheMemory_vec[m_version]->getMemoryValue(addr, value, size_in_bytes);
      found = true;
    } else if (Protocol::m_CMP && n->m_L2Cache_L2cacheMemory_vec[l2_ver]->tryCacheAccess(lineAddr, CacheRequestType_LD, dataPtr, 0)){
