...
# The following have to be synced with the BIOS
class bios_params_holder:
    def __init__(self):
        self.ebda_base = 0x9FC00
        self.rsdp_offs = 0x60
        # Memory map descriptors for INT 15h AX=E820.
        self.high_desc_offs = 0x90
        self.nvr_desc_offs = 0xA4
        self.reclaim_desc_offs = 0xB8
        self.high_desc_offs2 = 0xcc
        # SMEM is at EBDA:100h
        self.smem_offs = 0x100
...
def create_acpi_tables(build_acpi_tables, cpu_list, megs, ioapic_id,
                       user_rsdp_address, additional_tables = (),
                       bios_params = bios_params_holder()):

        mem = cpu_list[0].physical_memory

        if not build_acpi_tables:
                acpi_print("ACPI not enabled")
        ...
        if build_acpi_tables:
                ...
                madt_addr = dsdt_addr
        madt = create_madt(cpu_list, apic_addr, ioapic_addr, ioapic_id)
        if add_processors_in_ssdt:
                ssdt_addr = align_16(madt_addr + len(madt))
                ssdt = create_ssdt(cpu_list)
        ...
        smem_addr =         bios_params.ebda_base + bios_params.smem_offs
        rsdp_addr =         bios_params.ebda_base + bios_params.rsdp_offs
        high_desc_addr =    bios_params.ebda_base + bios_params.high_desc_offs
        nvr_desc_addr =     bios_params.ebda_base + bios_params.nvr_desc_offs
        reclaim_desc_addr = bios_params.ebda_base + bios_params.reclaim_desc_offs
        high_desc_addr2 =   bios_params.ebda_base + bios_params.high_desc_offs2
        ...
        smem = []
        append_dword(smem, megs * 1024L * 1024L)
        ...
        if roof_addr != top_of_memory:
                use_high_mem2 = 1
                high_mem_len2 = megs*1024L*1024L - high_mem_addr - high_mem_len - reclaim_len - nvr_len
        ...
        if build_acpi_tables:
                ...
                if user_rsdp_address:
                        write_table(mem, rsdp, user_rsdp_address)
