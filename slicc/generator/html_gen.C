...
function syncFrame () { \n \
  if (document.all && !window.opera) { \n \
    var scrollTop = Cabecera.document.body.scrollTop; \n \
    var scrollLeft = Cabecera.document.body.scrollLeft; \n \
  } \n \
  else { \n \
    var scrollTop = Cabecera.pageYOffset; \n \
  ...
  out << "<frameset rows=\"150,*,30\" ONLOAD=\"initScrollSynchronization()\">" << endl;
  Vector<StateMachine*> machine_vec = g_sym_table.getStateMachines();
 
 //JORGE
  if (machine_vec.size() > 1) {
    string machine = machine_vec[0]->getIdent();
    out << "  <frame name=\"Cabecera\" src=\"" << machine << "_cabecera.html\">" << endl;
  } else {
    out << "  <frame name=\"Cabecera\" src=\"cabecera.html\">" << endl;
  }
  if (machine_vec.size() > 1) {
    string machine = machine_vec[0]->getIdent();
