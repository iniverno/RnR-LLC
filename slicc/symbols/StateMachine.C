...
void StateMachine::writeHTMLFiles(string path) const
{
  ...
  */
  ...
  //Create Cabeceras
  //JORGE
  //creamos cabecera.html
  {
	
    ostringstream out;
    printHTMLCabeceras(out);
    
    // -- Write file
    filename = component + "_cabecera" + ".html";
    conditionally_write_file(path + filename, out);
  }
...
void StateMachine::printHTMLCabeceras(ostream& out) const
{
out << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Frameset//EN\" \"http://www.w3.org/TR/html4/frameset.dtd\">";
    out << "<HTML>";
     out << "<BODY>" << endl;
  ...
  // -- First column header
  out << "  <TH width=\"150\"> ---------- </TH>" << endl;

  for(int event = 0; event < numEvents(); event++ ) {
    out << "  <TH bgcolor=white>";
    out << frameRef(getIdent() + "_Event_" + getEvent(event).getIdent() + ".html", "Status", "2", formatHTMLShorthand(getEvent(event).getShorthand()));
    out << "</TH>" << endl;
  }
  out << "  <TH width=\"150\"> ------------ </TH>" << endl;
   out << "</TR>" << endl;
...
void StateMachine::printHTMLTransitions(ostream& out, int active_state) const
{
  ...
  for (int i=0; i<machine_vec.size(); i++) {
    ...
    if (type == this) {
     ...
     string javascript = " onclick=parent.frames[0].location='" + type->getIdent() +  "_cabecera.html\'";
      out << "<A target=\"Table\"href=\"" + type->getIdent() + "_table.html\"" + javascript + ">" + type->getIdent() + "</A>  " << endl;
    }
  }
  out << "</H1>" << endl;
  ...
  for(int state = 0; state < numStates(); state++ ) {
    ...
    out << frameRef(click_href, "Table", getIdent() + "_State_" + getState(state).getIdent() + ".html", "2", formatHTMLShorthand(getState(state).getShorthand()));
    out << "</TH>" << endl;
    ...
    for(int event = 0; event < numEvents(); event++ ) {
      ...
      if( trans_ptr != NULL ) {
        ...
        for (int action=0; action < actions.size(); action++) {
          ...
          actions_str += "  ";
          actions_str += frameRef(getIdent() + "_action_" + actions[action]->getIdent() + ".html", "Status", "2",
        ...
        if (trans_ptr->getNextStatePtr()->getIdent() != getState(state).getIdent()) { 
          ...
          nextState = frameRef(click_href, "Table", getIdent() + "_State_" + trans_ptr->getNextStatePtr()->getIdent() + ".html", "2", 
  ...
  for(int i = 0; i < numEvents(); i++ ) {
    out << "  <TH bgcolor=white>";
    out << frameRef(getIdent() + "_Event_" + getEvent(i).getIdent() + ".html", "Status", "2", formatHTMLShorthand(getEvent(i).getShorthand()));
    out << "</TH>" << endl;
  }
  out << "</TR>" << endl;
