...
void SymbolTable::writeChipFiles(string path) const
{
  ...
  // Create Chip.h
  {
	...
	sstr << "    void printTemp() const;" << endl;
...
//                   sstr << "  }\n";
	...
	sstr <<  "void Chip::printTemp() const" << endl;
	sstr <<  "{" << endl;
	sstr <<  "cerr << \"chip->printTemp\" << endl;" << endl;
	sstr <<  "/*  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {" << endl;
    sstr <<  "m_L2Cache_L2cacheMemory_vec[i]->printTempCommand(); " << endl;
    sstr <<  "}*/" << endl;
    sstr <<  "m_L2Cache_L2cacheMemory_vec[0]->printTempCommand();" << endl;
    sstr <<  "}" << endl << endl;

	sstr <<  "void Chip::resetTemp() const" << endl;
	sstr <<  "{" << endl;
	sstr <<  "cerr << \"chip->resetTemp\" << endl;" << endl;
	sstr <<  "/*  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {" << endl;
    sstr <<  "m_L2Cache_L2cacheMemory_vec[i]->printTempCommand(); " << endl;
    sstr <<  "}*/" << endl;
    sstr <<  "m_L2Cache_L2cacheMemory_vec[0]->resetTemp();" << endl;
    sstr <<  "}" << endl << endl;

	sstr <<  "void Chip::printReuse() const" << endl;
	sstr <<  "{" << endl;
	sstr <<  "cerr << \"chip->printReuse\" << endl;" << endl;
	sstr <<  "/*  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {" << endl;
    sstr <<  "m_L2Cache_L2cacheMemory_vec[i]->printReuseCommand(); " << endl;
    sstr <<  "}*/" << endl;
    sstr <<  "m_L2Cache_L2cacheMemory_vec[0]->printReuseCommand();" << endl;
    sstr <<  "}" << endl << endl;

	sstr <<  "void Chip::resetReuse() const" << endl;
	sstr <<  "{" << endl;
	sstr <<  "cerr << \"chip->resetReuse\" << endl;" << endl;
	sstr <<  "/*  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {" << endl;
    sstr <<  "m_L2Cache_L2cacheMemory_vec[i]->resetReuse(); " << endl;
    sstr <<  "}*/" << endl;
